...
module.exports = {
  command: '/gaddlead',
  aliases: ['/gaddlead'],
  description: 'Добавление роли Руководителя пользователю',
  async execute(context) {
    ...
    if (senderRoleId < 100) {
      return context.reply(`❌ У вас нет прав на выдачу роли Руководителя`);
    }
    ...
    try {
      ...
      for (const pool of selectResults) {
        ...
        for (const poolPeerId of poolPeerIds) {
		  ...
		  console.log(senderRoleIdd)

          if (senderRoleIdd < 100) {
            continue;
          }
        ...
        context.reply(`⭐ [id${label}|Пользователь] успешно получил роль «Руководитель»\n❓ Прошлая роль: «${currentUserRole}»`);
      }
    } catch (error) {
      console.error('Ошибка при добавлении роли:', error);
      return context.send('❌ Произошла ошибка.');
